...
- 成员邀请可选择有效时长和使用次数（1、5、10、50 次或有效期内不限次数），并查看剩余名额。
- 超级管理员可停用或重新启用用户；停用立即撤销登录，保留资料，空间唯一管理员须先交接管理权限。
...
PostgreSQL 的表和字段提供中文属性注释，已有数据库在应用启动迁移后生效；SQLite 不支持 COMMENT 属性，说明保留在模型中。升级兼容已安装的 `c3d8f7a9b641` 版本，保留原有账号、资源与邀请使用记录。
